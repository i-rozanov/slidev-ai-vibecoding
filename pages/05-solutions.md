...
<!--
Вообще есть два основных пути. Во-первых, концентрировать наиболее релевантную информацию в контексте и запросе, улучшать качество входа. Второй вариант - дробить один большой запрос на несколько маленьких, тем самым упрощая каждый из них и обеспечивая более релевантную информацию в каждом из запросов. Есть ещё и третий путь, по которому идут сами разработчики моделей, стараются всячески улучшать внимание, делать отдельные модели для программирования, лучше воспринимать инструкции, но здесь мы просто ждём новые модели.
-->
...
<!--
Ещё у некоторых агентов вы можете встретить функционал памяти, где отдельные промежуточные ответы/запросы складываются в отдельный файл и при необходимости добавляются в запрос, потому что как мы уже говорили, любой запрос является как у REST API stateless, то есть не запоминает состояние. В отдельных фреймворках для создания ИИ-агентов этим функционалом можно управлять. А так можно встретить в ChatGPT, недавно появилось у Qwen.
-->
...
<!--
А ещё разработчики быстро смекнули, что базовый контекст для кожаного мешка и ИИ-агента - разные, и использование README неоправдано для агентов, и придумали новый стандарт - файл AGENTS.md, этот файл есть в 60 тысячах репозиториев на гитхабе, посмотрим, возможно станет скоро неотъемлемой частью любого репозитория
-->
...
<!--
Alibaba представили в рамках своего IDE с AI агентом под названием Qoder представили интересную фичу, мне кажется, скоро она появится во всех агентах и станет базовым функционалом.
Во-первых, это создание вики. До первого запроса все файлы анализируются и генерится полная техническая информация о модулях, возможностях, оставляются примеры кодинга. Для 40 файлов это может занять порядка 2 часов.
Во-вторых, при создании задания генерится не ответ на вопрос, а спецификация на реализацию задания. Подтягиваются через RAG связанные вики-страницы и генерится подробная инструкция по реализации задачи, которую предлагается править, а не сам код.

Такой подход называется spec-first, считается очень прогрессивным, об этом чуть позже поговорим еще.
-->
...
<!--
И не могу не показать свой небольшой пет-проект по решению бэкендерских задач. Я постарался подумать, из чего обычно состоит декомпозиция, и используя Langchain организовать выполнение подзадач различными агентами, где условный создатель миграций для базы данных не получит стайлгайды по наименованию переменных в JSON. Реализовано пока процентов на 10%, я пока накидал как раз создатель миграций, обогатил его нашими правилами из docs.ensi.tech.
-->
...
<!--
Покажу, как это примерно программируется,[click] закидываем нашу локальную нейросеть,[click] запускаем тот самый цепочечный механизм с форматированным выводом
-->
...
<!--
Например, вот так выглядит форматированный вывод для агента, занимающегося декомпозицией задач [click], здесь задается enum для типов задач [click]
-->
...
<!--
Закидываем тестовое задание из базовых стажерных задач и получаем набор задач, по созданию моделей нам неинтересны [click], вот по созданию миграций [click]
-->
...
<!--
И такие задачи поступают на агента, у которого как раз есть только информация о том, как писать миграции в соответствии с подходами ensi, записываем эти правила и подгружаем их [click]. Тут хочу обратить ваше внимание на правило, что точность временных меток до микросекунд.[click]
-->
...
<!--
Получаем на выходе файл с миграцией, и, что ключевое, видим [click], что правила подцепились и метки у нас с точностью до микросекунд.
-->
