...
<!--
Давайте посмотрим, какие есть пути решения проблем
-->
...
<!--
Есть два основных пути. Во-первых, концентрировать наиболее релевантную информацию в контексте и запросе, улучшать качество входных данных. Второй вариант - дробить один большой запрос на несколько маленьких, тем самым упрощая каждый из них и обеспечивая более релевантную информацию в каждом из запросов. Есть ещё и третий путь, по которому идут сами разработчики моделей, стараются всячески улучшать внимание, делать отдельные модели для программирования, лучше воспринимать инструкции, но здесь мы просто ждём новые модели.
-->
...
<!--
У некоторых агентов есть функционал памяти: отдельные промежуточные ответы/запросы складываются в отдельный файл и при необходимости добавляются в запрос. Как мы уже говорили, любой запрос является stateless. В отдельных фреймворках для создания ИИ-агентов этим функционалом можно управлять. А так можно встретить в ChatGPT, недавно появилось у Qwen.
-->
...
<!--
А ещё разработчики быстро смекнули, что базовый контекст для кожаного мешка и ИИ-агента - разные. Использование README удобно для человеков, но не для агентов. Для них придумали новый стандарт - файл AGENTS.md. Этот файл есть в 60 тысячах репозиториев на гитхабе, посмотрим, возможно станет скоро неотъемлемой частью любого репозитория
-->
...
<!--
Alibaba представили в рамках своего IDE с AI агентом под названием Qoder интересную фичу. Мне кажется, скоро она появится во всех агентах и станет базовым функционалом.
...
Во-вторых, при создании задания генерится не ответ на вопрос, а спецификация на реализацию задания. Подтягиваются связанные вики-страницы, генерится подробная инструкция по реализации задачи, которую предлагается править разработчику, а затем по ней модель меняет сам код.
...
<!--
И не могу не показать свой небольшой пет-проект по решению бэкендерских задач. Я подумал, из чего обычно состоит декомпозиция, и используя Langchain организовал выполнение подзадач различными агентами, где условный создатель миграций для базы данных не получит стайлгайды по наименованию переменных в JSON. Реализовано сейчас около 10%, я набросал создатель миграций, обогатил его нашими правилами из docs.ensi.tech.
-->
...
<!--
Покажу, как это программируется.[click] Подключаем локальную нейросеть,[click] запускаем цепочечный механизм с форматированным выводом
-->
...
<!--
Вот так выглядит определение формата вывода для агента, занимающегося декомпозицией задач [click]. Здесь задается enum для типов задач [click]
-->
...
<!--
Закидываем тестовое задание из базовых стажерных задач и получаем набор задач. Создание моделей нам неинтересно [click], вот создание миграций [click]
-->
...
<!--
И такие задачи поступают на агента, у которого есть только информация о том, как писать миграции в соответствии с подходами ensi. Записываем эти правила и подгружаем их [click]. Тут хочу обратить ваше внимание на правило, что точность временных меток до микросекунд.[click]
-->
